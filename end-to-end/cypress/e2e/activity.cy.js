...
describe("Données d'activité", () => {
  ...
  it('Check that the doc button is accessible', () => {
    ...
    cy.get('.panel-helper .panel-header').get('.panel-header-closing-row').should('contain.text', "Données d'activité A-JUST :")
  ...
  it('Check that we can change period', () => {
    ...
    cy.get('aj-date-select').should('be.visible').click()
  ...
  it("Check that the 'Complétude gloabe' progress bar is displayed", () => {
    cy.get('.actions .left-panel')
      .get('.completion-bar').should('be.visible')
  })
