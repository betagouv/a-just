...
const KEEP_ARTIFACTS = !!(
  Cypress.env("KEEP_ARTIFACTS") || Cypress.env("CYPRESS_KEEP_ARTIFACTS")
);
...
function exportAndPersistActivite(
  baseUrl: string,
  startISO: string,
  stopISO: string,
  prefix: string,
          ...
          proto.click = function (this: HTMLAnchorElement) {
            try {
              ...
              if (href && /^blob:/i.test(String(href))) {
                (win as any).__downloadStarted = true;
                if (!(win as any).__lastDownloadName)
                  (win as any).__lastDownloadName = "blob.xlsx";
              }
            } catch {}
  ...
  cy.get("body").then(($body) => {
    ...
    if (
      $body.find("aj-extractor-activity > div.exportateur-container > div > p")
        .length
    ) {
      cy.get("aj-extractor-activity > div.exportateur-container > div > p", {
        timeout: 15000,
      })
        .scrollIntoView()
        .click({ force: true });
      ...
      cy.contains("button, [role=button], div", /Exporter|Excel|Télécharger/i, {
        timeout: 15000,
      })
        .scrollIntoView()
        .click({ force: true });
  ...
  cy.window({ log: false })
    .its("__downloadStarted", { timeout: 180000 })
    .should("be.oneOf", [true, false])
    .then((started) => {
      if (!started) {
        // Log last-known filename (may be empty if saveAs not called)
        cy.window({ log: false })
          .its("__lastDownloadName")
          .then((nm) => cy.log(`download.name: ${nm}`));
      }
    });
      ...
      cy.readFile(sbJson, { timeout: 60000 }).then((sb) => {
        cy.readFile(prJson, { timeout: 60000 }).then((pr) => {
          ...
          if (diffs.length > 0) {
            ...
            const report =
              `Type d'export : Activité\n` +
              `Référence : SANDBOX (${hostSB})\n` +
              `Candidat : PR (${hostPR})\n` +
              `Plage de dates : ${START} à ${STOP}\n\n` +
              summary;
            cy.writeFile(
              `cypress/artifacts/activite/diff_activite.txt`,
              report,
            );
