...
describe("Extracteur Collecte 2026 - Test de non-régression", () => {
  before(() => {
    // Rattache l'utilisateur E2E au backup TJ TEST avec les droits par défaut,
    // puis ouvre une session UI déjà authentifiée.
    return ensureE2EUserReady({ backupLabel: BACKUP_LABEL }).then(
      ({ ventilations }) => {
        cy.task(
          "log",
          `[SETUP] ${BACKUP_LABEL} assigned (backup id: ${ventilations.join(", ")})`,
        );
        cy.login();
        cy.visit("/panorama");
