...
describe("Extracteur Collecte 2026 - Test de non-régression", () => {
        ...
        .then((resp) => {
          ...
          return cy
            ...
            .then((allBackupsResp) => {
              cy.task(
                "log",
                `[DB DEBUG] API response type: ${typeof allBackupsResp.body}`,
              );
              cy.task(
                "log",
                `[DB DEBUG] API response keys: ${Object.keys(allBackupsResp.body || {}).join(", ")}`,
              );
              ...
              cy.task(
                "log",
                `[DB DEBUG] Found ${allBackups.length} backups total`,
              );
              cy.task(
                "log",
                `[DB DEBUG] First 5 backup labels: ${allBackups
                  .slice(0, 5)
                  .map((b: any) => b.label)
                  .join(", ")}`,
              );

              const e2eBackup = allBackups.find(
              ...
              if (!e2eBackup) {
                cy.task(
                  "log",
                  `[DB DEBUG] ✗ FATAL: ${BACKUP_LABEL} does not exist in database!`,
                );
                cy.task(
                  "log",
                  `[DB DEBUG] All backup labels: ${allBackups.map((b: any) => b.label).join(", ")}`,
                );
              ...
              cy.task(
                "log",
                `[DB DEBUG] ✓ Found ${BACKUP_LABEL} with ID: ${e2eBackup.id}`,
              );
              ...
              return cy.request({
                ...
                body: {
                  ...
                  ventilations: [e2eBackup.id], // Assign to TJ TEST backup
                  referentielIds: [],
                },
              });
            })
            .then(() => {
              ...
              return getUserDataApi(token).then((resp) => {
                ...
                return cy
                  .task("log", `[DB DEBUG] User ID: ${userId}`)
                  .then(() => {
                    return cy.task(
                      "log",
                      `[DB DEBUG] Found ${backups.length} backups in database:`,
                    );
                  })
                  .then(() => {
                    return cy.task(
        ...
        .then(() => {
          return cy
            ...
            .then(() => {
              return cy.task(
                "log",
                "[LOGIN] cy.login() completed successfully",
              );
            })
            .then(() => {
  ...
  it("Convert reference file to JSON", () => {
    ...
    const fixturePath =
      ...
      `/cypress/fixtures/${REFERENCE_FILE}`;
  ...
  it("Generate current extractor output", () => {
    ...
    cy.get("body").then(($body) => {
      ...
      cy.log(
        `[EXTRACTOR] Page title/heading contains: ${pageText.substring(0, 200)}`,
      );

      // Check for extractor-specific elements
      const hasExtractorVentilation =
        $body.find("aj-extractor-ventilation").length > 0;
      const hasExtractorActivity =
        $body.find("aj-extractor-activity").length > 0;
      const hasBackupSelector =
        $body.find('h6, [data-cy="backup-name"]').length > 0;

      cy.log(
        `[EXTRACTOR] aj-extractor-ventilation present: ${hasExtractorVentilation}`,
      );
      cy.log(
        `[EXTRACTOR] aj-extractor-activity present: ${hasExtractorActivity}`,
      );
