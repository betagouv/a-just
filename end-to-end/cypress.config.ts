...
export default defineConfig({
  ...
  e2e: {
    ...
    setupNodeEvents(on, config) {
      ...
      function writeBufferToDownloads({
        base64,
        fileName,
      }: {
        base64: string;
      ...
      function moveAndNormalizeXlsxTo({
        fileName,
        targetBase,
        subdir,
      }: {
        ...
        try {
          ...
          const srcPath = path.isAbsolute(fileName)
            ? fileName
            ...
            for (let r = range.s.r; r <= range.e.r; r++) {
              ...
              for (let c = range.s.c; c <= range.e.c; c++) {
                ...
                if (cell.s) {
                  ...
                  if (cell.s.font) {
                    style.font = {
                      ...
                      color: cell.s.font.color
                        ? cell.s.font.color.rgb || cell.s.font.color.theme || ""
                        : "",
                    };
                  }
                  ...
                  if (cell.s.fill) {
                    style.fill = {
                      fgColor: cell.s.fill.fgColor
                        ? cell.s.fill.fgColor.rgb ||
                          cell.s.fill.fgColor.theme ||
                          ""
                        : "",
                      bgColor: cell.s.fill.bgColor
                        ? cell.s.fill.bgColor.rgb ||
                          cell.s.fill.bgColor.theme ||
                          ""
                        : "",
                  ...
                  if (cell.s.border) {
                    style.border = {
                      top: cell.s.border.top
                        ? {
                            style: cell.s.border.top.style || "",
                            color: cell.s.border.top.color || "",
                          }
                        : null,
                      bottom: cell.s.border.bottom
                        ? {
                            style: cell.s.border.bottom.style || "",
                            color: cell.s.border.bottom.color || "",
                          }
                        : null,
                      left: cell.s.border.left
                        ? {
                            style: cell.s.border.left.style || "",
                            color: cell.s.border.left.color || "",
                          }
                        : null,
                      right: cell.s.border.right
                        ? {
                            style: cell.s.border.right.style || "",
                            color: cell.s.border.right.color || "",
                          }
                        : null,
                    };
                  }
                ...
                const validationKey = Object.keys(dataValidations).find(
                  (key) => {
                    try {
      ...
      function waitForDownloadedExcel({
        timeoutMs = 180000,
      }: {
        timeoutMs?: number;
      }) {
      ...
      function moveAndNormalizeXlsx({
        ...
        try {
          fs.mkdirSync(downloadsDir, { recursive: true });
          fs.mkdirSync(path.join(artifactsDir, "effectif"), {
            recursive: true,
          });
          const srcPath = path.isAbsolute(fileName)
            ? fileName
      ...
      const shouldWipe =
        !process.env.CYPRESS_KEEP_ARTIFACTS && !config.env?.KEEP_ARTIFACTS;
      if (shouldWipe) {
        try {
          wipeEffectifArtifacts();
        } catch {}
