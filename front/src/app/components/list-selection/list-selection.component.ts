...
export class ListSelectionComponent implements OnChanges {
   ...
   */
  @Output() valueChanged: EventEmitter<string | number | null> = new EventEmitter<string | number | null>()
  /**
   * Valeurs des ID sélectionnées changées
   */
  @Output() valuesChanged: EventEmitter<(string | number)[]> = new EventEmitter<(string | number)[]>()
  /**
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['value'] || changes['list'] || changes['values']) {
      if (this.multiple === false) {
        const value = this.value ? this.list.find((l) => l.id === this.value) || null : null

        this.itemsSelected = value ? [value] : []
      } else {
        this.itemsSelected = this.list.reduce((acc: ItemInterface[], cur: ItemInterface) => {
    ...
    if (this.values && this.values.length === this.list.length) this.labelPreview = this.labelPreviewText
  ...
  onSelectAll() {
    ...
    if (this.values.length === this.list.length) this.labelPreview = this.labelPreviewText
    else this.labelPreview = this.itemsSelected.map((i) => i.label).join(', ')
  }
}
