...
export class TextEditorComponent extends MainClass {
  ...
  ngOnChanges(change: SimpleChanges) {
    if (this.quillEditor) {
      if (change['askToModify'] && !this.defaultReadOnly) {
        console.log('askToModify', this.askToModify, this.defaultReadOnly)
        if (this.askToModify) {
        ...
        if (this.askToModify) {
          ...
          setTimeout(() => {
            if (this.quillEditor.getSelection()) this.quillEditor.setSelection(this.quillEditor.getSelection().index + 10, 0)
          }, 0)
          this.onFocus()
        }
      }
   ...
   */
  focus() {
    if (this.quillEditor && !this.defaultReadOnly) {
      this.quillEditor.enable()
      this.quillEditor.focus()
