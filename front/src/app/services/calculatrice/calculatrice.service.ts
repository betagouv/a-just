...
export class CalculatriceService {
  ...
  constructor() {
    this.dataCalculatrice.subscribe((calcul) => {
      ...
      if (
        typeof calcul.vacation.value === 'string' &&
        calcul.vacation.value.includes(',')
      ) {
      ...
      if (
        typeof calcul.vacation.unit === 'string' &&
        calcul.vacation.unit.includes(',')
      ) {
      ...
      if (
        typeof calcul.volume.value === 'string' &&
        calcul.volume.value.includes(',')
      ) {
  ...
  computeEtptCalculatrice(categoryId: string) {
    ...
    if (
      ...
      return (
        ((unit / basicEtptData[nbHoursPerDay]) *
          value *
          periodicData[cat][option]) /
        basicEtptData[byDayAndCat]
      );
      ...
      return (
        (value * periodicData[cat][option]) /
        (periodicData[cat]['jour'] * basicEtptData[nbHoursPerDay])
      );
    }
  }
