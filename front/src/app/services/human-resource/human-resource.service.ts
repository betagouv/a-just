...
export class HumanResourceService {
   ...
   */
  importedSituation: BehaviorSubject<importedSituation | null> =
    new BehaviorSubject<importedSituation | null>(null);
  /**
  ...
  getEtpByDateAndPerson(
    ...
    if (situation && situation.category && situation.category.id) {
      const activitiesFiltred = (situation.activities || []).filter(
        (a: any) => a.contentieux && a.contentieux.id === referentielId
      );
