  ...
  introSteps: IntroJSStep[] = this.isTJ()
    ? [
        ...
        },
        {
          target: '.step-3 > .title',
          title:
            "Le calendrier, le choix des contentieux et le nombre d'agents",
          intro:
        ...
        if (this.getOptionAffichageIndispoString()) {
          listFiltered = listFiltered.filter(
            (h) =>
              ...
              h.indisponibilities.some((i) =>
                (this.filterParams?.filterIndispoValues || []).includes(
                  i.contentieux.id
                )
              )
          );
