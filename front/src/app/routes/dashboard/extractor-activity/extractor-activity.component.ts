...
import { generalizeTimeZone, getShortMonthString, setTimeToMidDay } from '../../../utils/dates'
 ...
 * Excel file headers
 */
const headers = [' ', 'Période', 'Entrées logiciel', 'Entrées A-JUSTées', 'Sorties logiciel', 'Sorties A-JUSTées', 'Stock logiciel', 'Stock A-JUSTé']
...
export class ExtractorActivityComponent extends MainClass {
  ...
  constructor(
    ...
    this.minDate = this.userService.isCa() ? new Date('2022-01-01') : new Date('2021-01-01')
  }
   ...
   */
  checkValidity() {
    if (this.dateStart !== null && this.dateStop !== null) this.classSelected = ''
  ...
  getMonthTabName(act: any) {
    return getShortMonthString(new Date(act.periode)) + ' ' + new Date(act.periode).getFullYear()
  }
  ...
  getTotalPeriodeLabel(dateStart: Date, dateStop: Date, lowerStart = false) {
    return (
      (lowerStart ? 'de ' : 'De ') +
      (getShortMonthString(new Date(dateStart)) + ' ' + new Date(dateStart).getFullYear()) +
      ' à ' +
      (getShortMonthString(new Date(dateStop)) + ' ' + new Date(dateStop).getFullYear())
    )
  }
  ...
  getColumnWidth(headers: any, element: any) {
    return headers.map((header: any) => {
      return {
        wch: Math.max(
          header.length,
          element.reduce((w: any, r: any) => Math.max(w || 10, (r[header] || []).length || 10), 10),
        ),
      }
    })
  }
    ...
    return this.serverService
      ...
      .then((data) => {
        ...
        this.sumTab = this.sumTab
          .map((act: any) => {
            return this.generateFormatedDataMonth(act, this.getTotalPeriodeLabel(this.dateStart || new Date(), this.dateStop || new Date()), true)
          })
          .filter(
            (r: any) =>
              this.referentielService.idsIndispo.indexOf(r.idReferentiel) === -1 && this.referentielService.idsSoutien.indexOf(r.idReferentiel) === -1,
          )
        ...
        fetch('/assets/emptyTemplate_.xlsx')
          ...
          .then(async (report) => {
            ...
            if (this.sumTab.length === 0) {
              ...
              alert('Vous ne pouvez pas extraire plus de 4 années sur une même extraction.')
              throw 'no values'
            }
          ...
          .then((buffer) => {
            ...
            exposeDownloadToCypress(buffer as any, filename + EXCEL_EXTENSION)
  ...
  async getReport(report: any, viewModel: any) {
    report = this.setWidthForAllTabs(report, Object.keys(viewModel.data).length + 1)
    report = this.hideEmptyTabs(report, Object.keys(viewModel.data).length + 1)
    report.worksheets[0].name = 'Total sur la période '
    report = this.setHeaderText(report, 0, 2, ' ' + this.lowercaseFirstLetter(this.sumTab[0]['Période']) || '', true)
    ...
    this.data = Object.keys(this.data).map((key: any, index) => {
      ...
      this.data[key] = this.data[key]
        .map((act: any) => {
          report.worksheets[index + 1].conditionalFormattings = conditionnalFormating
          ...
          report = this.setHeaderText(report, index + 1, 2, ' sur ' + monthTabName)
          return this.generateFormatedDataMonth(act, monthTabName)
        })
        .filter(
          (r: any) => this.referentielService.idsIndispo.indexOf(r.idReferentiel) === -1 && this.referentielService.idsSoutien.indexOf(r.idReferentiel) === -1,
        )
  ...
  populateTab(report: any, data: any[], indexTab: number) {
    data.map((row: any, index: number) => {
      ...
      columnLetters.forEach((col, colIndex) => {
        ...
        if (keys[colIndex]) report.worksheets[indexTab + 1].getCell(`${col}${index + 3}`).value = row[keys[colIndex]]
      })
    })
  ...
  setHeaderText(report: any, tabIndex: number, headerIndex: number, sufix = '', sumTab = false) {
    const headers = [
      'Période',
      'Contentieux',
      'Total entrées logiciel',
    ...
    report.worksheets[tabIndex].autoFilter = 'A' + headerIndex + ':H' + headerIndex
    headers.map((elem, index) => {
      report.worksheets[tabIndex].getCell(this.getExcelColumnLetter(index) + headerIndex).value = elem
    })
  ...
  setWidthForAllTabs(report: any, loopIndex: number, sumTab = true) {
    ...
    do {
      ...
      do {
        if (index === 0) report.worksheets[loopIndex].columns[index].width = loopIndex === 0 ? 25 : 15
        else if (index === 1) report.worksheets[loopIndex].columns[index].width = 45
