  ...
  async filterList(ctx) {
    try {
      await withAbortTimeout(async (signal) => {
        let { backupId, date, fonctionsIds, categoryId, referentielList } =
          this.body(ctx);
        if (
        ...
        let referentiel = copyArray(
          await this.models.ContentieuxReferentiels.getReferentiels(backupId)
        ...
        checkAbort(signal);
        let hrfiltered = filterByCategoryAndFonction(
          copyArray(hr),
          null,
          fonctionsIds
        );
        ...
        const resultList = await Promise.all(categories.map(async (category) => {
          checkAbort(signal);
          const filterFonctionsIds =
            category.id === categoryId ? fonctionsIds : null;
          let allHr = preformatHumanResources(
            filterByCategoryAndFonction(
              copyArray(hr),
              category.id,
              filterFonctionsIds,
              date
          ...
          return {
            ...
            ), // force to filter by actual category
