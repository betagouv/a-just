...
export default class RouteContentieuxOptions extends Route {
  ...
  async getAll(ctx) {
    ...
    backupId = backups.find((b) => b.id === backupId)
      ? backupId
      : backups.length
  ...
  async duplicateBackup(ctx) {
    ...
    if (await this.models.OptionsBackups.haveAccess(backupId, juridictionId, ctx.state.user.id)) {
      this.sendOk(
        ctx,
        await this.model.models.OptionsBackups.duplicateBackup(backupId, backupName, juridictionId)
      );
    } else {
  ...
  async getBackupDetails(ctx) {
    ...
    if (
      await this.models.OptionsBackups.haveAccessWithoutJuridiction(backupId, ctx.state.user.id)
    ) {
