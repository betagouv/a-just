...
export default (sequelizeInstance, Model) => {
  ...
  Model.getListByHumanId = async (humanId, dateStart) => {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      if (
        !returnList.some(
          (r) => today(r.dateStart).getTime() === today(dateS).getTime()
        )
      ) {
        returnList.push({
          id: list[i].id,
  ...
  Model.syncSituations = async (list, humanId, cleanOldSituation = false) => {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      await Model.models.HRActivities.syncHRActivities(
  ...
  Model.destroySituationId = async (situationId) => {
    ...
    if (hr) {
      ...
      for (let i = 0; i < situations.length; i++) {
        if (
          today(situations[i].date_start).getTime() ===
          today(hr.date_start).getTime()
        ) {
