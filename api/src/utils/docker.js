...
export const selfRouteToSyncJuridiction = async (juridictionId) => {
  ...
  while (instancesFounded.length < config.nbInstances) {
    const newInstanceName = await axiosDt.post(
      `${config.serverUrl.replace("/api", "")}/docker/sync/update`,
      {
    ...
    if (
      newInstanceName.data &&
      instancesFounded.indexOf(newInstanceName.data) === -1
    ) {
