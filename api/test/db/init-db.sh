...
echo "Loading test database from test.sql.gz..."
gunzip -c /tmp/test.sql.gz | psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" 2>&1 | grep -v "invalid command" | grep -v "unrecognized configuration parameter" || true
