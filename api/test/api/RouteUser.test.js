  ...
  describe("Users tests", () => {
    ...
    it("Sign up - Correct inputs, should return 200", async () => {
      const response = await axios.post(
        `${config.serverUrl}/users/create-account`,
        {
          email: "test@mail.com",
          password: "123456",
          firstName: "userTest",
    ...
    it("Forgot password - Good email, should return 200", async () => {
      const response = await axios.post(
        `${config.serverUrl}/users/forgot-password`,
        {
          email: "test@mail.com",
        }
      );
    ...
    it("Get my datas as a connected user. Should return 200", async () => {
      const response = await axios.get(
        `${config.serverUrl}/users/get-user-datas`,
        {
          headers: {
            authorization: userToken,
          },
    ...
    it("Remove user Account by admin", async () => {
      const response = await axios.delete(
        `${config.serverUrl}/users/remove-account-test/${userId}`,
        {
          headers: {
            authorization: adminToken,
          },
